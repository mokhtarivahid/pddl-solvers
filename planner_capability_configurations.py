"""
Planner Capability Configurations Manager

Loads and manages planner capability rules and planner capability entries
from planner_capability_configurations.yaml.
"""
...
@dataclass
class PlannerCapabilityConfigurationCatalog:
    """Configuration catalog for compatibility rules and deterministic planner priority ordering."""
    ...
    @classmethod
    def default(cls) -> "PlannerCapabilityConfigurationCatalog":
    ...
    @classmethod
    def from_yaml_mapping(cls, data: Dict[str, Any]) -> "PlannerCapabilityConfigurationCatalog":
        catalog = cls.default()
...
class PlannerCapabilityConfigurations:
    """Loads planner capability configurations and planner entries from YAML."""

    def __init__(self, spec_file: Optional[str] = None):
        if spec_file is None:
            repo_root = Path(__file__).parent
            spec_file = repo_root / "planner_capability_configurations.yaml"

        self.spec_file = Path(spec_file)

    def load(self) -> Tuple[PlannerCapabilityConfigurationCatalog, Dict[str, Any], Optional[str]]:
        ...
        if yaml is None:
            return (
                PlannerCapabilityConfigurationCatalog.default(),
                {},
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError("planner_capability_configurations.yaml must contain a top-level mapping")

            catalog = PlannerCapabilityConfigurationCatalog.from_yaml_mapping(data)
...
PlannerCapabilityCatalog = PlannerCapabilityConfigurationCatalog
PlannerCapabilitiesSpecification = PlannerCapabilityConfigurations
